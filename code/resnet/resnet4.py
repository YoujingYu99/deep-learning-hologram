...
inp, re = load(PATH + 'train/concat/260concat.jpg')
...
train_dataset = tf.data.Dataset.list_files(PATH + 'train/concat/*.jpg')
train_dataset = train_dataset.map(load_image_train,
                                  num_parallel_calls=tf.data.experimental.AUTOTUNE)
train_dataset = train_dataset.shuffle(BUFFER_SIZE)
train_dataset = train_dataset.batch(BATCH_SIZE)

test_dataset = tf.data.Dataset.list_files(PATH + 'test/concat/*.jpg')
...
def Generator():
    ...
    skipsample.add(tf.keras.layers.Conv2D(K, 3, strides=1, padding='same',
                                          kernel_initializer=initializer, use_bias=False))
...
def Discriminator():
    ...
    down2 = downsample(filters=64, size=4, input_shape=[128, 128, 64], apply_batchnorm=False)(
        down1)  # (bs, 64, 64, 128)
    down3 = downsample(filters=64, size=4, input_shape=[64, 64, 64], apply_batchnorm=False)(down2)  # (bs, 32, 32, 256)
