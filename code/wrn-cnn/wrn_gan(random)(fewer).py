...
inp, re = load(PATH + 'random_train/0concat.jpg')
...
train_dataset = tf.data.Dataset.list_files(PATH + 'random_train/*.jpg')
train_dataset = train_dataset.map(load_image_train,
                                  num_parallel_calls=tf.data.experimental.AUTOTUNE)
train_dataset = train_dataset.shuffle(BUFFER_SIZE)
train_dataset = train_dataset.batch(BATCH_SIZE)

test_dataset = tf.data.Dataset.list_files(PATH + 'random_test/*.jpg')
...
def downsample(filters=K, size=5, input_shape=[], apply_batchnorm=True):
...
def upsample(filters=K, size=5, input_shape=[], apply_dropout=False):
    initializer = tf.random_normal_initializer(0., 0.02)
...
# up_model = upsample(filters=1, size=4, input_shape=[256, 256, 1])
...
def Generator():
    ...
    while n != 1:
        ...
        if n / 4 == 1:
            up_stack.append(upsample(input_shape=[int(n / 4), int(n / 4), K*k], filters=K*k))
        elif n / 4 == 64:
            up_stack.append(upsample(input_shape=[int(n / 4), int(n / 4), 2*K*k], filters=K))
        else:
            up_stack.append(upsample(input_shape=[int(n / 4), int(n / 4), 2*k*K], filters=K*k))
...
def Discriminator():
    ...
    down1 = downsample(filters=64, size=4, input_shape=[256, 256, 2], apply_batchnorm=False)(x)  # (bs, 128, 128, 64)

    down2 = downsample(filters=64, size=4, input_shape=[64, 64, 64], apply_batchnorm=False)(
        down1)  # (bs, 64, 64, 128)
    down3 = downsample(filters=64, size=4, input_shape=[16, 16, 64], apply_batchnorm=False)(down2)  # (bs, 32, 32, 256)
...
checkpoint_dir = './CNN-GAN_random_training_checkpoints'
